...
data class PartyMapOverviewPayload(
    ...
    val memberCount: Int,
    /** 0단계 — 전국을 하나로 묶은 칸. 핀이 하나도 없으면 null. */
    val country: MapCellPayload?,
    /** 1단계 — 시·도 단위. 핀이 찍힌 시·도만 담긴다. */
    val provinces: List<MapCellPayload>,
    /** 2·3단계 — 시·군·구 단위. 핀이 찍힌 지역만 담긴다. */
...
data class MapCellPayload(
    ...
    /** 이 칸에 내가 아직 기록하지 않은 핀이 하나라도 있으면 true. 회색 처리 + "탭해서 기록하기"의 조건이다. */
    val hasUnrecordedTrip: Boolean,
    /** 이 칸에서 가장 나중에 찍힌 핀의 등록 시각. "가장 최근 지역 1곳에만 툴팁"을 프론트가 고르는 기준이다. */
