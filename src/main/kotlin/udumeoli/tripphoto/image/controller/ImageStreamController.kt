...
class ImageStreamController(
    ...
    /**
     * 클라이언트(브라우저)에서 <img src="/api/images/{imageId}"> 로 호출할 때
    ...
    ): ResponseEntity<Void> = createRedirect(imageId, "original")

    @GetMapping("/api/images/{imageId}/thumbnail")
    fun redirectThumbnail(
        @LoginUser currentUserId: Long,
        @PathVariable imageId: Long,
    ): ResponseEntity<Void> = createRedirect(imageId, "thumb")
    ...
    private fun createRedirect(
        ...
        val targetUrl = "/stream/$imageId?type=$type&expires=$expires&sig=$sig"
